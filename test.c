...
int findNeighbor(char** arr, int** temp, char* word, int x, int y, int index) {
    ...
    for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++) {
            ...
            if (x_axis >= 0 && x_axis < bSize && y_axis >= 0 && y_axis < bSize && 
                *(*(arr + x_axis) + y_axis) == *(word + (index - 1))) {
                
                if ((*(*(temp + x_axis) + y_axis) % 10 != index - 1 || *(*(temp + x_axis) + y_axis) == 0)) {
                    int original = *(*(temp + x_axis) + y_axis);
                    if (original == 0) {
                        *(*(temp + x_axis) + y_axis) = index;
                    } else {
                        *(*(temp + x_axis) + y_axis) = original * 10 + index;
                    }
                    ...
                    if (original == 0) {
                        ...
                        *(*(temp + x_axis) + y_axis) = (original - index) / 10;
                    }
                }
            }
        }
